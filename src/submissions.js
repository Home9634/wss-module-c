...
async function loadSubmissions() {
    for (let i = 0; i < codeFiles.length; i++) {
        ...
        if (!fileName.includes(file.split('.')[0])) {
            continue
        }
        ...
        const fileDetails = fileName.split(".")[0].trim().split("_")
        let text = await response.text()
        text = text.split("\n")

        const submission = {}

        submission["Passed"] = text[0].split(":")[1].trim().split("/")[0]
        submission["Total"] = text[0].split(":")[1].trim().split("/")[1]
        text.shift()
        submission["Answer"] = text.join("\n")

        const name = fileDetails[1].split("-").join(" ")
        const [year, month, day, hours, minutes, seconds] = fileDetails[2].split("-")

        const date = new Date(year, month, day, hours, minutes, seconds)

        submission["Name"] = name
        submission["Date"] = `${year}-${month}-${day}`
        submission["Time"] = `${hours}:${minutes}:${seconds}`
...
function renderSubmissions() {
    const submissionsTable = document.getElementById("submissions-container")

    submissions.forEach((submission, index) => {
        console.log(submission)
        submissionsTable.innerHTML += `
            <div>
                <div class="submission-title" onclick="toggleSubmission('${index}')">${submission.Name} (${submission.Date} ${submission.Time})</div>      
                <div id="code-${index}" class="code-display hidden">
                    <div class="cases-container">${getResults(submission.Passed, submission.Total)}</div>
...
function toggleSubmission(index) {
    document.getElementById(`code-${index}`).classList.toggle("hidden")
}
...
function getResults(passed, total) {
    ...
    if (failed > 0) {
        content += `<div class="case case-failed"></div>`
    }
...
function viewStatement() {
    window.location = `statement.html?file=${file}`
}
